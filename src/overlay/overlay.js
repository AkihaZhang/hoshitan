(function () {
  ...
  function onCharEnter(ev) {
    ...
    showPopup(anchor, preview.text, '<div class="loading">' + escapeHtml(tr('Looking up…')) + '</div>');
	  ...
	  function definitionTextForEntry(entry) {
	    const term = entry && entry.term ? entry.term : {};
	    const glossaries = Array.isArray(term.glossaries) ? term.glossaries : [];
	    return glossaries.slice(0, Math.max(1, Number(state.config.maxGlossesPerEntry || 4))).map(glossary => {
	      const raw = String((glossary && glossary.glossary) || '');
	      const parsed = parseGlossaryJson(raw);
	      const text = normalizeWhitespace(parsed ? plainTextFromNode(parsed) : raw);
	      const dict = normalizeWhitespace(glossary && glossary.dict);
	      return text ? (dict ? '[' + dict + '] ' + text : text) : '';
	    }).filter(Boolean).join('\n\n').slice(0, 30000);
	  }
	  function requestAnkiExport(button) {
	    ...
	    const payload = {
	      ...
	      definition: definitionTextForEntry(entry),
	      dictionary: (Array.isArray(term.glossaries) ? term.glossaries : []).map(g => String((g && g.dict) || '')).filter(Boolean).join(', '),
	      at: Date.now()
	    };
