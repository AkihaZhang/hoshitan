(function () {
  ...
  const OVERLAY_MESSAGES = {
    'zh-CN': {
      'Rendering…': '正在渲染…',
      'Looking up…': '正在查词…',
      'Could not find the selected dictionary entry.': '找不到所选词典词条。',
      'Adding...': '正在添加...',
      'Creating Anki card...': '正在创建 Anki 卡片...',
      'Add to Anki': '添加到 Anki',
      'Could not send Anki request: {error}': '无法发送 Anki 请求：{error}',
      'Grammar': '语法',
      'Etymology': '词源',
      'Details': '详情',
      'Inflection': '词形变化',
      ...
      'Pitch': '音调',
      'Lookup failed': '查词失败',
      'No dictionary entry found from this word.': '未找到该词的词典条目。',
      'No dictionary entry found from this character.': '未找到从该字符开始的词典条目。',
      'looked up from: {surface}': '查询原文：{surface}',
      'Working…': '处理中…',
      'Done': '完成',
      'Failed': '失败',
      'Done.': '已完成。',
  ...
  function overlayUiLanguage() {
    const configured = String(state.config.uiLanguage || 'auto').toLowerCase().replace(/_/g, '-');
    if (configured === 'zh-cn' || configured === 'zh-hans') return 'zh-CN';
    if (configured === 'en') return 'en';
    const resolved = String(state.config.resolvedUiLanguage || '').toLowerCase().replace(/_/g, '-');
    if (resolved === 'zh-cn' || resolved === 'zh-hans') return 'zh-CN';
    try {
  ...
  function tr(key, values) {
    const table = OVERLAY_MESSAGES[overlayUiLanguage()] || {};
  ...
  function localizedSectionLabel(label) {
    const raw = String(label || 'Details');
  ...
  const AUDIO_PROBE_STABLE_MS = 420;
	  ...
	  function audioSourceDisplayLabel(source, index) {
	    ...
	    if (/^(?:https?:\/\/)?(?:127\.0\.0\.1|localhost):(?:5050|19742)(?:[/?#]|$)/i.test(url)) return 'Local audio';
	    try {
	      if (typeof URL === 'function') {
	  ...
	  function applyCustomPopupCss(cssText) {
	    ...
	    try {
	      if (!customPopupStyleEl) {
	        ...
	        customPopupStyleEl.id = 'hoshitan-custom-popup-css';
  ...
  function overlayDebug(message) {
    ...
    try { console.log('[hoshitan overlay] ' + String(message || '')); } catch (_) {}
  }
  ...
  function applyConfig(config) {
    ...
    try { document.documentElement.lang = overlayUiLanguage() === 'zh-CN' ? 'zh-Hans' : 'en'; } catch (_) {}
  ...
  function onCharEnter(ev) {
    ...
    if (stored) {
      ...
      showPopup(anchor, preview.text, '<div class="loading">' + escapeHtml(tr('Rendering…')) + '</div>');
      renderStoredLookup(stored);
      return;
    }
    activateMatchRange(preview.start, preview.text || anchor.textContent || '');
    requestLookupFromPlugin(pos);
    showPopup(anchor, preview.text, '<div class="loading">' + escapeHtml(tr('Looking up…')) + '</div>');
  }
	  ...
	  function glossaryPlainText(glossary) {
	    ...
	    return normalizeWhitespace(parsed ? plainTextFromNode(parsed) : raw);
	  }
	  function glossaryDataForEntry(entry) {
	  ...
	  function furiganaPlainForEntry(entry) {
	    ...
	    if (!reading || !expression || compareTextKey(expression) === compareTextKey(reading)) return expression;
	    return expression + '[' + reading + ']';
	  }
	  function frequencyDataForEntry(entry) {
	    const term = entry && entry.term ? entry.term : {};
	    const groups = Array.isArray(term.frequencies) ? term.frequencies : [];
	    const lines = [];
	    const ranks = [];
	    const seenDictionaries = Object.create(null);
	    groups.forEach(group => {
	      const dictionary = normalizeWhitespace(group && (group.dict || group.dictName || group.dictionary));
	      const values = Array.isArray(group && group.frequencies) ? group.frequencies : [];
	      const display = values.map(value => normalizeWhitespace(
	        (value && (value.displayValue || value.display_value)) ||
	        (value && value.value !== undefined ? String(value.value) : '')
	      )).filter(Boolean);
	      if (dictionary || display.length) lines.push((dictionary ? dictionary + ': ' : '') + display.join(', '));
	      if (dictionary && seenDictionaries[dictionary]) return;
	      if (dictionary) seenDictionaries[dictionary] = true;
	      const first = values[0];
	      const raw = first && ((first.displayValue || first.display_value) !== undefined
	        ? (first.displayValue || first.display_value)
	        : first.value);
	      const match = String(raw === undefined || raw === null ? '' : raw).match(/^\s*(\d+)/);
	      const rank = match ? Number(match[1]) : Number(raw);
	      if (Number.isFinite(rank) && rank > 0) ranks.push(rank);
	    });
	    const harmonic = ranks.length
	      ? String(Math.floor(ranks.length / ranks.reduce((sum, rank) => sum + (1 / rank), 0)))
	      : '';
	    return { frequencies: lines.join('\n'), frequencyHarmonicRank: harmonic };
	  }
	  function pitchDataForEntry(entry) {
	    const term = entry && entry.term ? entry.term : {};
	    const groups = Array.isArray(term.pitches) ? term.pitches : [];
	    const positions = [];
	    groups.forEach(group => {
	      const values = Array.isArray(group && group.positions)
	        ? group.positions
	        : (Array.isArray(group && group.pitchPositions)
	          ? group.pitchPositions
	          : (Array.isArray(group && group.pitch_positions) ? group.pitch_positions : []));
	      values.forEach(value => {
	        const position = Number(value);
	        if (Number.isFinite(position) && positions.indexOf(position) < 0) positions.push(position);
	      });
	    });
	    const rules = Array.isArray(term.rules) ? term.rules : String(term.rules || '').split(/\s+/);
	    const verbOrAdjective = rules.some(rule => /^(?:v|adj-i)/i.test(String(rule || '')));
	    const moraCount = splitJapaneseMoras(String(term.reading || term.expression || '')).length;
	    const categories = [];
	    positions.forEach(position => {
	      let category = '';
	      if (position === 0) category = 'heiban';
	      else if (verbOrAdjective) category = 'kifuku';
	      else if (position === 1) category = 'atamadaka';
	      else if (position > 1) category = position >= moraCount ? 'odaka' : 'nakadaka';
	      if (category && categories.indexOf(category) < 0) categories.push(category);
	    });
	    return {
	      pitchAccentPositions: positions.map(position => '[' + String(position) + ']').join('\n'),
	      pitchAccentCategories: categories.join(',')
	    };
	  }
	  function popupSelectionText() {
	    try {
	      const selection = window.getSelection && window.getSelection();
	      return normalizeWhitespace(selection && selection.toString ? selection.toString() : '');
	    } catch (_) {
	      return '';
	    }
	  }
	  ...
	  function ankiPayloadForEntry(entry, requestId) {
	    ...
	    const frequency = frequencyDataForEntry(entry);
	    const pitch = pitchDataForEntry(entry);
	    return {
	      type: 'anki-add',
	      requestId,
	      lineId: state.lineId,
	      ...
	      dictionary: Object.keys(glossary.singleGlossaries).join(', '),
	      at: Date.now()
	    };
	  }
	  ...
	  function requestAnkiExport(button) {
	    ...
	    if (!entry) {
	      setStatus({ message: tr('Could not find the selected dictionary entry.'), kind: 'error' });
	      return;
	    }
	    if (state.ankiPendingRequestId) return;
	    const requestId = 'anki-' + String(Date.now()) + '-' + String(++state.ankiRequestSeq);
	    const payload = ankiPayloadForEntry(entry, requestId);
	    ...
	    if (!sent) {
	      try { iina.postMessage('anki-add', payload); }
	      catch (error) {
  ...
  function ensureBridgeSocket() {
    ...
    try {
      ...
      socket.onclose = () => {
        try { console.log('[hoshitan overlay] bridge socket close'); } catch (_) {}
      ...
      socket.onerror = () => {
        try { console.log('[hoshitan overlay] bridge socket error'); } catch (_) {}
  ...
  function sendBridgeMessage(payload) {
    ...
    if (socket && socket.readyState === WebSocket.OPEN) {
      try { socket.send(JSON.stringify(payload)); return true; } catch (error) { try { console.log('[hoshitan overlay] bridge send failed ' + String(error)); } catch (_) {} }
    }
    return false;
  }
  ...
  function placePopup(anchor) {
    ...
    const gap = Math.max(4, Number(state.config.popupSubtitleGapPx || 8));
  ...
  function fallbackGlossaryText(raw) {
    ...
    return bits.length ? bits.join('\n') : s;
	  ...
	  function renderGrammarHtml(content, ctx) {
	    ...
	    return '<div class="grammar-row"><b>' + escapeHtml(tr('Grammar')) + '</b>: <span>' + inline + '</span></div>';
	  }
	  function renderGrammarText(text, ctx) {
	    const value = normalizeWhitespace(String(text || '').replace(/^[:\s]+/, ''));
	    if (!value) return '';
	    overlayDebug("detected grammar section source=" + String(ctx && ctx.sourceKind || "generic"));
	    return '<div class="grammar-row"><b>' + escapeHtml(tr('Grammar')) + '</b>: <span>' + escapeAndLinkifyText(value) + '</span></div>';
	  }
	  ...
	  function cleanupNonLemmaText(text) {
	    ...
	    let raw = String(text || '').replace(/\r/g, '\n').replace(/https?:\/\/[^\s<>"']+/gi, url => {
	      const token = '__HOSHITAN_URL_' + urls.length + '__';
	      urls.push(url);
	      return token;
	    });
	    ...
	    return raw.split(/\n+/).map(part => {
	      ...
	      urls.forEach((url, index) => { restored = restored.replace('__HOSHITAN_URL_' + index + '__', url); });
	      return restored;
	    }).filter(Boolean);
	  }
	  function renderNonLemmaText(text, ctx) {
	    ...
	    return rows.map(part => {
	      const label = NONLEMMA_GRAMMAR_WORD_RE.test(part) ? tr('Inflection') : tr('Definition');
	  ...
	  function renderWiktionaryPairTupleNonLemma(parsed, glossaryItem, ctx) {
	    ...
	    return '<div class="nonlemma-list">' + rows.map(row =>
	      '<div class="nonlemma-row"><b>' + escapeHtml(tr('Form of')) + '</b>: <span>' +
	  ...
	  function renderCollapsibleSection(label, bodyHtml, open, className) {
	    ...
	    return '<details class="dict-details' + cls + '"' + (open ? ' open' : '') + '><summary>' + escapeHtml(localizedSectionLabel(label || 'Details')) + '</summary><div class="details-body">' + body + '</div></details>';
	  }
	  ...
	  function renderXrefBox(node, ctx) {
	    ...
	    const linkNodes = findNodes(node, n => n && n.tag === 'a');
	    const label = labelNode ? plainTextFromNode(labelNode.content) : tr('See also');
	    const links = linkNodes.map(n => renderInlineNode(n, ctx)).filter(Boolean);
	  ...
	  function renderEntryMetadata(term) {
	    ...
	    frequencies.forEach(entry => {
	      ...
	      chips.push('<span class="freq-chip" title="' + escapeHtml((dict || tr('Frequency')) + (display ? ' ' + display : '')) + '"><span class="meta-label">' + escapeHtml(dict || tr('Frequency')) + '</span>' + (display ? ' ' + escapeHtml(display) : '') + '</span>');
	    });
	    ...
	    pitches.forEach(entry => {
	      ...
	      chips.push('<span class="pitch-group" title="' + escapeHtml((dict || tr('Pitch')) + (titleDisplay ? ' ' + titleDisplay : '')) + '"><span class="pitch-source-chip">' + escapeHtml(dict || tr('Pitch')) + '</span><span class="pitch-patterns">' + display + '</span></span>');
	    });
	  ...
	  function displayHeaderForResult(result, firstEntry) {
	    ...
	    const secondary = surface && compareTextKey(surface) !== compareTextKey(heading)
	      ? tr('looked up from: {surface}', { surface })
	      : '';
  ...
  function renderTaskPanel() {
    ...
    taskEl.innerHTML =
      '<div class="task-head"><div class="task-title">' + escapeHtml(task.title || tr('Working…')) + '</div><div class="task-elapsed">' + escapeHtml(elapsed) + '</div></div>' +
  ...
  function setTaskStatus(payload) {
    ...
    if (payload.active === false) {
      ...
      state.task = Object.assign({}, existing, payload, {
        ...
        title: existing.title || payload.title || (payload.success ? tr('Done') : tr('Failed')),
        message: payload.message || (payload.success ? tr('Done.') : tr('Failed.')),
  ...
  iina.onMessage('anki-export-result', payload => {
    if (!payload || String(payload.requestId || '') !== state.ankiPendingRequestId) return;
    const button = state.ankiPendingButton;
    state.ankiPendingRequestId = '';
    state.ankiPendingButton = null;
    if (button) {
      button.disabled = false;
