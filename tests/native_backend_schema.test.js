...
const nativeSource = fs.readFileSync(path.join(root, 'src/native/iina_hoshi.cpp'), 'utf8');
assert(/append_term_metadata_json/.test(nativeSource), 'Native bridge should serialize term metadata');
assert(/\\?"frequencies\\?"/.test(nativeSource), 'Native bridge should emit frequency metadata');
assert(/\\?"pitches\\?"/.test(nativeSource), 'Native bridge should emit pitch metadata');
assert(/add_freq_dict/.test(nativeSource), 'Native bridge should load frequency dictionaries');
...
assert(/vendor\/hoshidicts\/include\/hoshidicts\/query\.hpp/.test(buildScript), 'Submodule validation should check HoshiDicts headers');
assert(/git submodule update --init --recursive/.test(buildScript), 'Submodule validation should give the initialization command');
